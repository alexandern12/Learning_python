...
def draw_grid(w, draw_rows, surface):
    size_between = w // draw_rows

    x = 0
    y = 0
    for ll in range(draw_rows):
        x = x + size_between
        y = y + size_between
...
def random_snack(random_rows, item):
    positions = item.body
...
def main():
    global width, rows, ss, snack
    width = 500
    rows = 20
    win = pygame.display.set_mode((width, width))
    ss = Snake((255, 0, 0), (10, 10))
    snack = Cube(random_snack(rows, ss), color=(0, 255, 0))
    flag = True
    ...
    while flag:
        ...
        for x in range(len(ss.body)):
            if ss.body[x].pos in list(map(lambda z: z.pos, ss.body[x + 1:])):
